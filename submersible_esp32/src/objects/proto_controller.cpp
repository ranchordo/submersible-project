...
struct InStreamParser {
    InStreamParser(uint8_t* instream, uint32_t available) {
...
IRAM_ATTR void proto_timer_isr() {
    ...
    if (arg->comms->getInstream()[last_packet_start + 16] == 0) {
        ...
        InStreamParser parser(arg->comms->getInstream(), available);
...
ProtocolController::ProtocolController(CommunicationSubsystem* comms) {
    ...
    if (s) {
        ...
        timerAttachInterrupt(periodic_timer.timer, &proto_timer_isr, true);
